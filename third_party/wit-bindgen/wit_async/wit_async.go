...
func FutureOrStreamWait(code uint32, handle int32) (uint32, uint32) {
	if code == RETURN_CODE_BLOCKED {
		if state == nil {
			code = waitSynchronously(uint32(handle))
		} else {
			if state.waitableSet == 0 {
				state.waitableSet = waitableSetNew()
			}
...
func waitSynchronously(handle uint32) uint32 {
	waitableSet := waitableSetNew()
	defer waitableSetDrop(waitableSet)
	waitableJoin(handle, waitableSet)
	...
	for {
		event0 := waitableSetPoll(waitableSet, buffer)
		event := unsafe.Slice((*uint32)(buffer), 2)
		event1 := event[0]
		event2 := event[1]
		...
		case EVENT_STREAM_READ, EVENT_STREAM_WRITE, EVENT_FUTURE_READ, EVENT_FUTURE_WRITE:
			...
			waitableJoin(handle, 0)
			return event2

		case EVENT_SUBTASK:
			// No subtask state exists in synchronous mode, ignore and keep polling.
			continue

		default:
			panic(fmt.Sprintf("unexpected event: %v", event0))
		}
	}
}
